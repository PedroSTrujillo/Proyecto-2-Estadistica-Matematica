...
### Teorema del Límite Central (TLC) para el EMV y estadístico $\chi^2$
...
Por lo visto anteriormente, para un tamaño muestral fijo $n$ y $m=1000$ réplicas, el TLC para $\hat\theta$ implica que $\mathrm{Cov}(\hat\theta)\approx \mathcal{I}_F(\theta_0)^{-1}/n$. De ahí que, si calculamos la covarianza muestral entre $(\hat\alpha,\hat\beta)$ a partir de las $m$ réplicas y la multiplicamos por $n$, deberíamos obtener algo cercano a $\mathcal{I}_F(\theta_0)^{-1}$.

Para verificar lo anterior,se computa, para cada $n$: (1) $n\,\widehat{\mathrm{Cov}}(\hat\theta)$; (2) la diferencia con $\mathcal{I}_F(\theta_0)^{-1}$; y (3) medidas de error relativas para comentar la convergencia. Esto se puede ver en el siguiente código:
...
# --- Supuestos previos del notebook ---
stopifnot(exists("I_F"), exists("resultados"), exists("alpha_0"), exists("beta_0"))
theta0     <- c(alpha_0, beta_0)
IF0        <- I_F(theta0)
Sigma_theo <- solve(IF0)                 # I_F^{-1}(theta0)
Cor_theo   <- cov2cor(Sigma_theo)

# --- Cálculo por tamaño muestral ---
res_cov_tbl2 <- split(resultados, resultados$n) |>
  lapply(function(df) {
    n_i    <- df$n[1]
    S_hat  <- stats::cov(cbind(df$alpha_hat, df$beta_hat))      # Cov muestral entre réplicas
    nS_hat <- n_i * S_hat                                       # n * Cov_muestral
    Diff   <- nS_hat - Sigma_theo

    # Evitar divisiones por cero si alguna entrada teórica fuese 0 (no ocurre aquí, pero es robusto)
    denom <- pmax(abs(Sigma_theo), .Machine$double.eps)

    data.frame(
      n = n_i,
      rel_frob      = norm(Diff, type = "F") / norm(Sigma_theo, type = "F"),
      rel_max       = max(abs(Diff) / denom),
      cor_max_diff  = max(abs(cov2cor(nS_hat) - Cor_theo)),
      err11         = (nS_hat[1,1] / Sigma_theo[1,1]) - 1,
      err12         = (nS_hat[1,2] / Sigma_theo[1,2]) - 1,
      err22         = (nS_hat[2,2] / Sigma_theo[2,2]) - 1
    )
  }) |>
  bind_rows() |>
  arrange(n)

# --- Tabla 1: resumen de discrepancias globales ---
tabla_resumen <- res_cov_tbl2 |>
  transmute(
    n,
    `Frobenius (%)` = 100 * rel_frob,
    `Máx. (%)`      = 100 * rel_max,
    `|Δ correl| máx.`  = cor_max_diff
  )

tabla_resumen |>
  kbl(booktabs = TRUE, digits = c(0, 2, 2, 4),
      col.names = c("n","Frobenius (%)","Máx. (%)","|Δ correl| máx."),
      caption   = "Resumen de discrepancias") |>
  kable_paper("hover", full_width = FALSE) |>
  column_spec(1, bold = TRUE, width = "4em") |>
  row_spec(0, background = "#0f172a", color = "white")

# --- Tabla 2: errores relativos por entrada (en %) ---
tabla_entradas <- res_cov_tbl2 |>
  transmute(
    n,
    `E11 (%)` = 100 * err11,
    `E12 (%)` = 100 * err12,
...
tabla_entradas |>
  kbl(booktabs = TRUE, digits = c(0, 1, 1, 1),
      caption = "Errores relativos por entrada: (n·Cov_emp − $I_F^{-1}$) / $I_F^{-1}$ × 100%.") |>
  kable_paper("hover", full_width = FALSE) |>
  add_header_above(c(" " = 1, "Errores (%)" = 3)) |>
  row_spec(0, background = "#0f172a", color = "white")
```

A partir de estos resultados, al comparar $n\,\widehat{\mathrm{Cov}}(\hat\theta)$ con $\mathcal{I}_F(\theta_0)^{-1}$ se observa que los errores globales son pequeños y disminuyen marcadamente de $n=100$ a $n=500$: el error relativo de Frobenius pasa de **5.37%** a **2.25%**, y el máximo error por entrada de **13.54%** a **2.61%**.

Más allá, entrada a entrada el mayor desajuste aparece en la varianza de $\hat\alpha$ para $n=100$ (**E11 ≈ +13.5%**), mientras que para $n\ge 500$ todas las entradas quedan dentro de **±3%**. Además, la diferencia máxima entre matrices de correlación cae de $\sim 1.1\times10^{-2}$ a $\sim 10^{-3}$, lo que indica que también converge la **orientación** de la elipse.

En conjunto, esto es coherente con la convergencia $n\,\widehat{\mathrm{Cov}}(\hat\theta)\to \mathcal{I}_F(\theta_0)^{-1}$; las leves oscilaciones entre $n=500,1000,2000$ se explican por el **error Monte Carlo** con $m=1000$ réplicas. Por último, con un criterio práctico (errores $\lesssim 3\%$ y $|\Delta\mathrm{correl}|\lesssim 0.01$), la aproximación es **buena para** $n\ge 500$ y **muy buena** para $n=1000$–$2000$. No obstante, para $n=100$–$200$ persiste una sub/sobre-estimación de algunas varianzas (hasta $\sim 10$–$14\%$ en E11 a $n=100$), esperable por el carácter asintótico del resultado.
